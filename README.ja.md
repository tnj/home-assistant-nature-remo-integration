...
| --- | --- |
| エアコン | `climate` — 運転モード、設定温度、風量、上下・左右スイング。固定ボタン(スイング/チルト等)は `button`、リモコン側ステートは 二値=`switch`(内部クリーン、エコ等)、多値=`select`(湿度レベル等)、時刻=`time`(ナイトセットモード等)。API が機種ごとに返すリモコン側ステートをそのまま公開します(ダイキン・三菱・パナソニックのリモコンで確認済み) |
...
- 家電本体の物理リモコンによる状態変更は **エアコンに限り** 反映されます。
  [2026 年 7 月](https://nature.global/blog/27763/)から、Nature Remo 3 /
  Lapis / mini 2 / nano をエアコン室内機の近くに設置していれば、リモコンの
  赤外線を Remo が受信して状態を同期します。同期された状態は他の変更と同じく
  次回ポーリングで本インテグレーションに反映されます(2026-08-03 に実測: ダイキンの
  リモコンでの温度・スイング変更が数秒で Cloud API に現れました)。エアコン以外
  — テレビ、照明、学習リモコン — を物理リモコンで操作した場合は検知できません。
