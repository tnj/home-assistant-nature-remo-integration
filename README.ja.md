...
| --- | --- |
| エアコン | `climate` — 運転モード、設定温度、風量、上下・左右スイング。固定ボタン(スイング/チルト等)は `button`、リモコン側ステートは 二値=`switch`(内部クリーン等)、多値=`select`(加湿レベル等)、時刻=`time`(ナイトセットモード等) |
| 床暖房 | `climate` — auto / warm(暖房)モード、設定温度。リモコン側ステート(省エネ等)は 二値=`switch`、多値=`select`、時刻=`time` |
...
| プロジェクター | リモコン画面のキーごとに 1 つの `button`(電源のみ既定で有効、他はワンクリックで有効化) |
...
- 一部のエアコンや床暖房はオートモードで相対温度(`-2`〜`+2`)を返しますが、そのまま数値として
  表示します。
- リモコン側ステート(内部クリーン、省エネ等)は運転モードによって使用可否が変わります。
  現在のモードで使えないものは対応するエンティティが unavailable になります。書き込んでも
  クラウド側で受理されたうえで無視されるためです。
...
- デバイス・家電およびそれらのエンティティ(学習リモコン信号、テレビ/照明のボタン、
  リモコン側ステート)は Nature アカウントの状態に自動追従します。新規に追加された
  ものは再起動なしで出現し、Nature アプリ側で削除されたものはおよそ 3 回分のポーリング
  (約 3 分)後にエンティティレジストリから自動的に削除され、使えない孤児エンティティ
  として残り続けることはありません。
